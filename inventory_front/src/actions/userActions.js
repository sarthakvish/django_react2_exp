import {
    ...
    USER_UPDATE_PROFILE_SUCCESS, USER_UPDATE_PROFILE_RESET, USER_DETAILS_RESET,
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
    try{
        ...
        const {data}=await axios.put(`http://localhost:3000/api/users/profile/update/`,
            user,
            config)
...
/*        Once profile update, we get back new profile information and we want user to logged
        in with this new information, so need to trigger login success also*/
        ...
        /*Then also we want to update local storage*/
        localStorage.setItem('userInfo', JSON.stringify(data))
    }catch (error){
        dispatch({
...
export const deleteUser = (id) => async (dispatch, getState) => {
    try{
        ...
        const {data}=await axios.delete(`http://localhost:3000/api/users/delete/${id}`,
            config)
